...
public class DistanceEnemy : Enemy {
    ...
    void Update()
    {
        ...
        RaycastHit2D hit = Physics2D.Raycast(transform.position + new Vector3(-1, -0.1f, 0), Vector2.left, 7f);

        if (hit.collider != null)
        {
            if (hit.collider.gameObject.tag == "Character" && !disparando)
            {
                Atacar();
                disparando = true;
                atacking = true;
    ...
    public override void Atacar()
    {
        ...
        proyectil.gameObject.GetComponent<FlechaE>().damage = atack;
        StartCoroutine(idle());
    }
